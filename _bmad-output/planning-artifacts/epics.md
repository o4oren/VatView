...
**And** swiping to full reveals the complete card: individual controller details (rating, time online per position), raw METAR string fetched inline from `metar.vatsim.net` and displayed under a METAR label (no navigation redirect), full ATIS text per station (supports A/D ATIS split)
